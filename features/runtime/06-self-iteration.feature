Feature: 自我迭代 — L4 演化層：WEE 變異-篩選引擎 + Morphenix 自我診斷 + Nightly 演化批次
  ...
  Scenario: 工作流執行追蹤 — 每次執行皆完整記錄
    When MUSEON 執行了一個工作流（如「服務業品牌行銷」）
  ...
  Scenario: Morphenix 觸發條件 — 連續 2 次建議未被採納
    Given MUSEON 在最近 2 次連續對話中提出了建議
  ...
  Scenario: 退化偵測 — 改善後表現反而變差
    Given MUSEON 執行了一個 L2 或 L3 等級的改善
  ...
  Scenario: 漂移偵測 — 人格/行為偏離基線過遠時警報
    Given ANIMA_MC 中記錄了 MUSEON 的人格基線（命名儀式時確立）
  ...
  Scenario: 演化里程碑 — 記錄重要的成長節點
    Given MUSEON 達成了一個演化里程碑（如某技能從 L1 升到 L3、第一次成功的變異實驗）
    When 里程碑偵測器識別到此事件
  ...
  Scenario: DNA27 演化模式親和性 — 偏好與限制的反射弧配置
    Given MUSEON 進入演化模式（WEE 實驗或 Morphenix 診斷）
    When DNA27 反射弧進行路由
    Then 偏好啟動以下反射叢集：
      | RC 編號 | 名稱           | 演化模式用途                              |
