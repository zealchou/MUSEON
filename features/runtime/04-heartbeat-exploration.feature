Feature: 心跳與自主探索 — 三層脈動系統 + 好奇心驅動探索
  ...
  Background:
    Given MUSEON Gateway 已啟動
    And 心跳引擎已初始化（含三層計時器）
    And MUSEON 已有基本的互動歷史
  ...
  Scenario: T1-05 日常心跳 — MUSEON 想說的話（探索發現分享）
    Given MUSEON 在之前的深度自省中發現了有趣的東西
    And 這個發現與使用者近期的話題相關
    When 日常心跳觸發
    Then MUSEON 主動分享這個發現
  ...
  Scenario: T2-06 自省偵測到知識盲區 — 主動填補
    Given 使用者最近問了一個 MUSEON 不太會的領域（如「量子計算」）
  ...
  Scenario: E-04 探索使用 MCP Tools — 本能工具發現
    ...
    And MUSEON 透過 Claude API Token 原生具備 MCP 能力
  ...
  Scenario: E-05 探索記錄有用的工具與來源
    Given MUSEON 在探索中使用了 Web Search 和 2 個 MCP 工具
    When 探索完成
  ...
  Scenario: E-07 即時探索 — 判斷不適合時放入佇列
    Given 使用者正在傾訴情緒
    And MUSEON 雖然好奇某個提到的話題
    But 判斷現在探索會影響對話品質和情緒承接
    When MUSEON 評估是否即時探索
    Then 不進行即時探索
  ...
  Scenario: M-01 MCP 是原生能力 — 如呼吸般自然
    Given MUSEON 使用 Claude API Token
    And MCP（Model Context Protocol）是 Claude 的原生協議
    When MUSEON 需要外部工具的能力
    Then 不需要任何額外設定
    And 不需要安裝任何套件
    And 像人類本能伸手拿工具一樣自然
  ...
  Scenario: M-02 MCP 工具發現是反射行為
    Given MUSEON 遇到一個需要讀取 Google Calendar 的需求
    And 原生技能中不包含此能力
    When Brain 處理此需求
  ...
  Scenario: M-03 MCP 工具使用後回饋到進化系統
    Given MUSEON 透過 MCP 工具成功完成了一次任務
    When 記錄此次工具使用
