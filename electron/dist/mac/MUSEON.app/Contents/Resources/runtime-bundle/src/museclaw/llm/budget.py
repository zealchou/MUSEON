...
class BudgetMonitor:
    ...
    def __init__(
        ...
        Args:
            daily_limit: Daily token limit (0 = read from env MUSECLAW_DAILY_TOKEN_LIMIT, default 200000)
            warning_threshold: Percentage (0-1) at which to warn
            data_dir: Path to data directory for persistence (e.g., ~/MuseClaw 正式版/MuseClaw/data)
        """
        if daily_limit <= 0:
            env_limit = os.environ.get("MUSECLAW_DAILY_TOKEN_LIMIT", "200000")
            try:
    ...
    def set_daily_limit(self, new_limit: int) -> None:
        ...
        Also persists to MUSECLAW_DAILY_TOKEN_LIMIT env var.
        ...
        os.environ["MUSECLAW_DAILY_TOKEN_LIMIT"] = str(new_limit)
