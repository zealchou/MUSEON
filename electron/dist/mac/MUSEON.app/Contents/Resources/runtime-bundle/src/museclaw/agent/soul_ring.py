"""靈魂年輪 — append-only 成長記錄系統.

靈魂年輪（Soul Ring）是 MuseClaw 不可逆的成長痕跡：
  每一次認知突破、服務里程碑、失敗教訓、價值校準
  都被刻入 ANIMA_MC.L4.soul_rings[] 作為 append-only 記錄。
年輪如同樹的年輪——只進不退、不可竄改。

同時 ANIMA_USER 也有使用者側的觀察年輪（observation_rings），
兩套年輪的交互構成 MuseClaw 的完整成長敘事。
...
class SoulRing:
    ...
    靈魂年輪是 MuseClaw 的不可逆成長記錄，一旦寫入不可修改或刪除。
...
class ObservationRing:
    ...
    Attributes:
        ...
        resonance_link: 與 MuseClaw soul_ring 的共振連結
    """
...
class RingDepositor:
    ...
    def detect_ring_event(
        ...
        偵測四種信號：
          1. 認知框架變化（推理路徑結構性改變）
          2. 服務品質顯著提升（Q-Score 新高或新任務類型完成）
          3. 明確的失敗（使用者指出錯誤或 Q-Score 低於閾值）
          4. 價值觀校正（使用者使用 Kernel 五大價值觀關鍵詞）

        Args:
            user_content: 使用者訊息文字
            response_content: MuseClaw 回覆文字
        ...
        # 信號 2：服務品質顯著提升
        if q_score is not None and q_score_history:
            historical_max = max(q_score_history) if q_score_history else 0.0
    ...
    def create_resonance_link(
        ...
        """建立 MuseClaw soul_ring 與 ANIMA_USER observation_ring 之間的共振連結.

        Args:
            soul_ring_hash: MuseClaw 靈魂年輪的 hash
