Feature: 靈魂年輪 — append-only 成長記錄、SHA-256 完整性鏈與雙 ANIMA 年輪交互
  ...
  Background:
    Given MuseClaw 已完成命名儀式且 ANIMA_MC.json 已初始化
  ...
  Scenario: cognitive_breakthrough — 認知突破型年輪
    Given MuseClaw 在與使用者的對話中發現一個前所未有的洞見
    And 該洞見改變了 MuseClaw 對某個領域的理解框架
  ...
  Scenario: value_calibration — 價值校準型年輪
    Given 使用者（老闆 Zeal）對 MuseClaw 的價值觀表達進行了校正
  ...
  Scenario: 年輪共振 — MuseClaw 的成長影響使用者觀察
    Given MuseClaw 的 soul_ring 記錄了一次認知突破
  ...
  Scenario: 年輪回顧 — 定期回顧成長軌跡
    ...
    Then 系統生成「成長軌跡報告」包含：
    And 本週新增的 MuseClaw soul_rings 摘要
  ...
  Scenario: 安裝更新時年輪不被覆蓋
    When 使用者執行 Install-MuseClaw.command 進行更新安裝
    Then data/anima/soul_rings.json 被視為使用者資料
    And 安裝過程備份並還原年輪檔案
    And 更新後年輪完整性驗證通過
    And 年輪是 MuseClaw 最珍貴的資產——絕不可遺失
