Feature: 自解壓安裝包 — Self-Extracting Installer Packaging
  ...
  Scenario: 收集來源檔案 — 包含必要目錄
    ...
    And 應該包含以下檔案
      ...
      | Install-MuseClaw.command   | 專案內安裝腳本    |
  ...
  Scenario: tar.gz 內容結構正確
    Given tar.gz 已建立
    When 列出 tar.gz 內容
    Then 應該包含 src/museclaw/__init__.py
  ...
  Scenario: 自解壓標頭 — 安裝流程完整
    ...
    And 應該設定 MUSECLAW_HOME 為安裝根目錄
    And 應該啟動 BDD 安裝流程（python -m museclaw.installer）
  ...
  Scenario: 載荷提取往返測試
    ...
    When 從 .command 提取載荷
    And 解碼並解壓到臨時目錄
    Then 臨時目錄應該包含 src/museclaw/__init__.py
