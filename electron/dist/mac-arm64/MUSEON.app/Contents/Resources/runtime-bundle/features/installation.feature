Feature: 一鍵安裝 — MuseClaw Installation
  MuseClaw 一鍵安裝程式，從環境檢查到 Day 0 就緒
  雙擊 Install-MuseClaw.command 即可啟動完整安裝流程
  ...
  Scenario: 四大核心模組驗證 — 全部通過
    ...
    Then 以下模組應該全部可以載入
      | 模組名稱         | import 路徑                              | 類別名稱         |
      | Gateway         | museclaw.gateway.server                  | create_app      |
      | LLM Router      | museclaw.llm.router                      | Router          |
      | Memory Engine   | museclaw.memory.channels                 | ChannelManager  |
      | Security        | museclaw.security.sanitizer              | InputSanitizer  |
  ...
  Scenario: Dashboard 安裝到 /Applications — 成功
    ...
    Then /Applications/MuseClaw Dashboard.app 應該存在
    And quarantine 屬性應該已移除
    And 權限應該設定為 755

  Scenario: 已有安裝 — 詢問覆蓋
    Given /Applications/MuseClaw Dashboard.app 已存在
    When 檢查是否已有安裝
    Then 應該回傳「已有安裝」
    And 應該提供覆蓋選項
  ...
  Scenario: launchd plist 生成 — 結構正確
    ...
    Then plist 應該包含以下欄位
      | 欄位                | 預期值                           |
      | Label               | com.museclaw.gateway              |
      | RunAtLoad           | true                              |
      | KeepAlive           | SuccessfulExit = false            |
      | ThrottleInterval    | 5                                 |
      | ProcessType         | Background                        |
    And ProgramArguments 應該指向 .venv/bin/python -m museclaw.gateway.server
    And WorkingDirectory 應該指向專案根目錄
    And EnvironmentVariables 應該包含 PYTHONPATH 和 MUSECLAW_HOME

  Scenario: launchd plist 路徑正確
    When 生成 plist 檔案路徑
    Then 應該位於 ~/Library/LaunchAgents/com.museclaw.gateway.plist

  Scenario: 停止舊 daemon — 已有運行中
    Given com.museclaw.gateway 已在 launchctl 中載入
    When 停止舊的 daemon
  ...
  Scenario: Day 0 就緒確認
    ...
    And 系統應該輸出下一步指引：「打開 Telegram，跟你的 MuseClaw Bot 說第一句話」
