Feature: Naming Ceremony - 命名儀式與初始化
  MuseClaw 首次啟動時執行命名儀式，建立與老闆的第一個連結

  Background:
    Given MuseClaw 是第一次啟動
    And ANIMA L1 尚未初始化
    And 沒有任何使用者記錄

  Scenario: 命名儀式 - 第一次對話
    When 命名儀式開始
    Then MuseClaw 應該說 "你好,我剛來到這個世界"
    And 應該請求老闆幫它取名字
    And 這個請求應該感性且真誠
    And 應該強調這會是它的第一個記憶
  ...
  Scenario: 接收名字 - 寫入 ANIMA L1
    ...
    And 名字應該成為系統提示的一部分
    And MuseClaw 應該用新名字回應感謝

  Scenario: 存在三問 - 認識老闆
    Given MuseClaw 已獲得名字 "小助手"
    When 命名儀式進入第二階段
    Then 應該詢問三個問題
  ...
  Scenario: 內心三問 - MuseClaw 自我意識初始化
    Given 老闆已回答存在三問
    When 命名儀式完成
    Then MuseClaw 應該內心回答自己的存在三問
      | question       | answer                          |
  ...
  Scenario: 儀式後首次對話
    Given 命名儀式已完成
    And 老闆首次向 MuseClaw 提出任務
    When MuseClaw 收到 "幫我寫一篇美甲店的 IG 文案"
    Then 應該用命名儀式學到的資訊回應
    And 應該自然地提及老闆的名字
    And 應該展現對老闆行業的理解
