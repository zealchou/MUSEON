Feature: 技能鍛造（ACSF）與 MCP 原生本能 — 外部槓桿、反射弧鑄造與技能生命週期
  作為 MuseClaw（一個運行於 Claude API Token 上的 MUSEON AI OS 實例）
  ...
  Background:
    ...
    And MuseClaw 運行在 Claude API Token 上（MCP 是原生能力，無需額外設定）
  ...
  Scenario: MCP 工具類別涵蓋——MuseClaw 可本能觸及的外部能力範圍
    Given MuseClaw 運行在 Claude API Token 上
  ...
  Scenario: 外部工具優先規則——MCP 搜尋失敗後才考慮其他路徑
    Given 使用者的需求需要 MuseClaw 不具備的能力
  ...
  Scenario: MCP 工具使用紀錄——每次使用都留下痕跡
    Given MuseClaw 透過 MCP 成功調用了 Google Calendar 工具
    When 工具調用完成
  ...
  Scenario: MCP 工具使用失敗的處理——不好用的工具會被記住
    Given MuseClaw 調用某 MCP 工具但連續失敗 3 次
    When 第三次失敗時
  ...
  Scenario: MCP 工具使用不影響 DNA27 治理——安全護欄始終生效
    Given MuseClaw 正在透過 MCP 工具讀取使用者的郵件
  ...
  Scenario: 鍛造前先做市場研究——不重複造輪子
    Given MuseClaw 確認 MCP 工具和外部資源都不足以滿足需求
    And 決定需要鍛造一個新技能
    When 啟動鍛造流程
