...
class ToolExecutor:
    ...
    def __init__(
        ...
        try:
            from museclaw.agent.mcp_connector import MCPConnectorSDK
    ...
    def _find_whisper_cli(self) -> Optional[Path]:
        ...
        base_dirs = [
            ...
            Path.home() / "MuseClaw" / "data",
            Path.home() / "MuseClaw" / ".runtime" / "data",
        ]
    ...
    async def _execute_generate_artifact(
        ...
        try:
            ...
            from museclaw.gateway.message import Artifact
            artifact = Artifact(
                type=artifact_type,
    ...
    async def _execute_shell_exec(
        ...
        try:
            ...
            # 允許 workspace 及其子目錄，也允許 MUSECLAW_HOME 下的路徑
            import os
            museclaw_home = os.environ.get(
                "MUSECLAW_HOME", str(_Path.home() / "MUSEON")
            )
            museclaw_root = _Path(museclaw_home).resolve()

            wd_ok = (
                str(resolved_wd).startswith(str(workspace_root))
                or str(resolved_wd).startswith(str(museclaw_root))
            )
            if not wd_ok:
                logger.warning(
                    f"shell_exec working_dir 逃逸攔截: {working_dir} "
                    f"（不在 workspace {workspace_root} 或 "
                    f"MUSECLAW_HOME {museclaw_root} 範圍內）"
                )
        ...
        try:
            ...
            import os
            try:
                from museclaw.security.env_security import sanitize_shell_env
            ...
            import shlex
            try:
                from museclaw.security.trusted_bins import (
