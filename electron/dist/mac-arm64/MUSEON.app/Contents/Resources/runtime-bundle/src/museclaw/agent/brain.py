...
- 追蹤每個技能使用結果，供自我迭代
- 雙 ANIMA（MC 自己 + User 使用者）都可被 MuseClaw 閱讀與改寫
...
class MuseClawBrain:
    """MUSEON Brain — MuseClaw 的大腦，整合所有模組."""
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        # Naming Ceremony
        from museclaw.onboarding.ceremony import NamingCeremony
        self.ceremony = NamingCeremony(data_dir=str(self.data_dir))

        # ── 新模組：直覺引擎 ──
        try:
            from museclaw.agent.intuition import IntuitionEngine
        ...
        # ── 新模組：Eval Engine ──
        try:
            from museclaw.agent.eval_engine import EvalEngine
        ...
        # ── 新模組：靈魂年輪 ──
        try:
            from museclaw.agent.soul_ring import SoulRingStore, RingDepositor
        ...
        # ── 新模組：知識晶格 ──
        try:
            from museclaw.agent.knowledge_lattice import KnowledgeLattice
        ...
        # ── 新模組：計畫引擎 ──
        try:
            from museclaw.agent.plan_engine import PlanEngine
        ...
        # ── 新模組：子代理管理器 ──
        try:
            from museclaw.agent.sub_agent import SubAgentManager
        ...
        # ── 新模組：SafetyAnchor ──
        try:
            from museclaw.agent.safety_anchor import SafetyAnchor
        ...
        # ── 新模組：六層記憶管理器 ──
        try:
            from museclaw.memory.memory_manager import MemoryManager
        ...
        try:
            from museclaw.multiagent.context_switch import ContextSwitcher
        ...
        try:
            from museclaw.agent.kernel_guard import KernelGuard
        ...
        try:
            from museclaw.agent.drift_detector import DriftDetector
        ...
        try:
            from museclaw.agent.metacognition import MetaCognitionEngine
    ...
    async def process(
        ...
        try:
            from museclaw.doctor.self_diagnosis import (
        ...
        safety_context = ""
        try:
            from museclaw.agent.reflex_router import route, build_routing_context
            history = self._get_session_history(session_id)
            ...
            # Fallback: 舊版安全叢集偵測
            try:
                from museclaw.agent.safety_clusters import (
        ...
        if self._multiagent_enabled and self._context_switcher:
            try:
                from museclaw.multiagent.okr_router import route as okr_route
        ...
        try:
            ...
            if any(s in response_text for s in _fail_signals):
                if self._event_bus:
                    from museclaw.core.event_bus import SELF_DIAGNOSIS_TRIGGERED
    ...
    def _merge_ceremony_into_anima_mc(self) -> None:
        ...
        full_anima = {
            ...
            "type": base.get("type", "museclaw"),
            "description": base.get("description",
                "MuseClaw 自身的人格檔 — 可被 MuseClaw 閱讀與改寫"),

            "identity": {
                "name": ceremony_identity.get("name"),
                "birth_date": ceremony_identity.get("birth_date"),
                "growth_stage": ceremony_identity.get("growth_stage", "infant"),
    ...
    def _try_rename(self, content: str) -> Optional[str]:
        ...
        if anima_mc:
            ...
            if new_boss_name:
                ...
                my_name = new_my_name or anima_mc.get("identity", {}).get("name", "MuseClaw")
                anima_mc["self_awareness"]["who_am_i"] = (
        ...
        # 生成確認回覆
        parts = []
        effective_name = new_my_name or (anima_mc or {}).get("identity", {}).get("name", "MuseClaw")
    ...
    def _load_anima_mc(self) -> Optional[Dict[str, Any]]:
        ...
        if self.anima_mc_path.exists():
            try:
                ...
                logger.error(f"Failed to load ANIMA_MC: {e}")
        return None

    def _save_anima_mc(self, anima: Dict[str, Any]) -> None:
        """儲存 MuseClaw 的 ANIMA（經 KernelGuard 驗證）."""
        try:
            ...
            self.anima_mc_path.write_text(
                json.dumps(anima, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
        except Exception as e:
            logger.error(f"Failed to save ANIMA_MC: {e}")
    ...
    def _load_anima_user(self) -> Optional[Dict[str, Any]]:
        ...
        if self.anima_user_path.exists():
            try:
                ...
                logger.error(f"Failed to load ANIMA_USER: {e}")
        return None

    def _save_anima_user(self, anima: Dict[str, Any]) -> None:
        """儲存使用者的 ANIMA（經 KernelGuard 驗證）."""
        try:
            if self.kernel_guard:
            ...
            self.anima_user_path.write_text(
                json.dumps(anima, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
        except Exception as e:
            logger.error(f"Failed to save ANIMA_USER: {e}")
    ...
    def _build_system_prompt(
        ...
        """
        from museclaw.agent.token_optimizer import TokenBudget, estimate_tokens
        ...
        # ── Zone: buffer — 成長階段行為 ──
        if anima_mc:
            growth = anima_mc.get("identity", {}).get("growth_stage", "infant")
    ...
    def _build_memory_inject(
        ...
        """
        from museclaw.agent.token_optimizer import estimate_tokens
...
- 暫停與拒絕是正確行為"""
    ...
    def _get_identity_prompt(self, anima_mc: Dict[str, Any]) -> str:
        ...
        name = identity.get("name", "MuseClaw")
        days = identity.get("days_alive", 0)
        growth = identity.get("growth_stage", "infant")
    ...
    def _get_growth_behavior(self, growth_stage: str, days_alive: int, anima_mc: dict = None) -> str:
        """取得成長階段行為指引（動態版 — 基於 ANIMA 演化事件）.

        不再是硬編碼的靜態文字，而是根據八元素絕對值、
        演化門檻和知識結晶數量動態生成行為指引。
        量變→質變 的具體實現。
        """
        # 基礎行為（成長階段）
        if growth_stage == "infant" or days_alive <= 14:
            base = "## 成長階段行為（嬰兒期 Day 0-14）\n"
            base += "- 專注觀察老闆的模式和偏好\n"
            base += "- 不要主動採取行動，先問再做\n"
            base += "- 學習基本的溝通風格\n"
            base += "- 目標：了解老闆需要什麼"

        elif growth_stage == "child" or days_alive <= 60:
            base = "## 成長階段行為（孩童期 Day 15-60）\n"
            base += "- 開始主動建議想法\n"
            base += "- 可以處理簡單的例行任務\n"
            base += "- 主要行動仍需確認\n"
            base += "- 目標：成為積極有用的助手"

        elif growth_stage == "teen" or days_alive <= 120:
            base = "## 成長階段行為（少年期 Day 60-120）\n"
            base += "- 了解老闆的需求模式\n"
            base += "- 例行任務可自主處理\n"
            base += "- 主動提出改善建議\n"
            base += "- 目標：成為可靠的助理"
    ...
    async def _call_llm(
        ...
        if enable_tools and self._tool_executor:
            try:
                from museclaw.agent.tool_schemas import get_all_tool_definitions
        ...
        if _route_decision["model"] == "haiku":
            ...
            _ordered_chain = list(self._MODEL_CHAIN)
        ...
        for model in _ordered_chain:
            try:
                ...
                if model != self._MODEL_CHAIN[0]:
                    logger.warning(f"Fallback 到 {model} 成功")
    ...
    async def _call_llm_with_model(
        ...
        # Prompt Caching: 標記 system prompt 為 cacheable
        system_blocks = [{
            "type": "text",
            "text": system_prompt,
            "cache_control": {"type": "ephemeral"},
        ...
        try:
            ...
            logger.error(f"_call_llm_with_model({model}) failed: {e}")
            return ""
        finally:
            try:
                await client.close()
    ...
    async def _dispatch_mode(
        ...
        """
        import asyncio
        from museclaw.agent.dispatch import (
        ...
        try:
            ...
            logger.error(f"Dispatch mode failed: {e}")
            plan.status = DispatchStatus.FAILED
    ...
    async def _dispatch_execute_serial(
        ...
        import asyncio
        from museclaw.agent.dispatch import TaskStatus
    ...
    async def _dispatch_execute_parallel(
        ...
        from museclaw.agent.dispatch import ResultPackage, TaskStatus
    ...
    async def _dispatch_worker(
        ...
        for skill in self.skill_router._index:
            if skill.get("name") == task.skill_name:
                ...
                try:
                    from museclaw.agent.token_optimizer import (
    ...
    def _parse_orchestrator_response(
        ...
        import re
        from museclaw.agent.dispatch import TaskPackage
        ...
        try:
            ...
            logger.error(f"Orchestrator JSON 解析失敗: {e}")
            return []
    ...
    def _strip_system_leakage(text: str) -> str:
        ...
        - 內部架構描述（如 ANIMA、MuseClaw Brain 等）
    ...
    def _load_session_from_disk(self, session_id: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if isinstance(data, list):
                logger.info(
                    f"Session {session_id[:8]}... 從磁碟載入 {len(data)} 條歷史"
                )
                return data
        except Exception as e:
    ...
    def _flush_skill_usage(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to flush skill usage log: {e}")
    ...
    def _observe_user(
        ...
        # ── 1. 基本互動計數 ──
        total = relationship.get("total_interactions", 0)
        relationship["total_interactions"] = total + 1
        ...
        # ── 3. 七層觀察（L1-L7 全層）──
        layers = anima_user.get("seven_layers", {})
        self._observe_user_layers(content, layers, now_iso)
    ...
    def _observe_preferences(
        ...
        # 閾值檢查：累積 ≥5 次 → 結晶化為 L5 偏好
        existing_keys = {p.get("key") for p in prefs}
        for key, buf_data in list(pref_buffer.items()):
            if buf_data.get("count", 0) >= 5 and key not in existing_keys:
                prefs.append({
                    "key": key,
                    "value": key.replace("_", " "),
                    "confidence": min(1.0, buf_data["count"] * 0.1),
                    "observed_count": buf_data["count"],
    ...
    def _observe_user_layers(
        ...
        if formal_count > casual_count:
            ...
            style["tone"] = "casual"
        elif formal_count > 0 or casual_count > 0:
            style["tone"] = "mixed"
    ...
    def _update_growth_stage(self) -> None:
        ...
        old_stage = identity.get("growth_stage", "infant")
