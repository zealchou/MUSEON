...
TOOL_CONFIGS: Dict[str, ToolConfig] = {
    "searxng": ToolConfig(
        ...
        extra_config={
            "settings_yml": {
                ...
                "server": {
                    "secret_key": "museclaw-searxng-secret",
                    "limiter": False,
                },
                "search": {
                    "safe_search": 0,
    ...
    "dify": ToolConfig(
        ...
        display_name="Dify",
        emoji="🔄",
        description="工作流引擎 — MuseClaw 的手腳，視覺化 AI 工作流建構平台，自動化排程與跨系統整合",
        category="automation",
        install_type="compose",
        cost_tier="cpu",
        ...
        extra_config={
            ...
            "integration": "MuseClaw 透過 REST API 自主執行工作流",
        },
    ),
}
...
class ToolRegistry:
    ...
    def _install_docker(
        ...
        try:
            ...
            if name == "qdrant":
                run_args.extend([
                    "-v", f"museclaw-qdrant-data:/qdrant/storage",
    ...
    def _install_firecrawl_compose(
        ...
        try:
            ...
            compose_content = (
                ...
                "    container_name: museclaw-firecrawl-redis\n"
                "    restart: unless-stopped\n"
                "    networks: [firecrawl]\n"
                "  playwright:\n"
                "    image: ghcr.io/firecrawl/playwright-service:latest\n"
                "    container_name: museclaw-firecrawl-playwright\n"
                "    restart: unless-stopped\n"
                ...
                "    container_name: museclaw-firecrawl-api\n"
                "    restart: unless-stopped\n"
                "    environment:\n"
                ...
                "      - SELF_HOSTED_WEBHOOK_SECRET=museclaw\n"
                "    ports:\n"
                "      - '127.0.0.1:3002:3002'\n"
                ...
                "  worker:\n"
                "    image: ghcr.io/firecrawl/firecrawl:latest\n"
                "    container_name: museclaw-firecrawl-worker\n"
                "    restart: unless-stopped\n"
                "    environment:\n"
                "      - REDIS_URL=redis://redis:6379\n"
                "      - PLAYWRIGHT_MICROSERVICE_URL=http://playwright:3000\n"
    ...
    def auto_detect(self) -> Dict:
        ...
        for name, config in TOOL_CONFIGS.items():
            ...
            if config.install_type == "docker":
                ...
                try:
                    result = subprocess.run(
                        ["docker", "inspect", f"museclaw-{name}"],
                        capture_output=True, text=True, timeout=10,
                    )
                    if result.returncode == 0:
                        state.installed = True
