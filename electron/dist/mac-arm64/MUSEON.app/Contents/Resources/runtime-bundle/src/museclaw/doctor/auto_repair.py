"""MuseClaw Doctor — 自動修復引擎（Layer 2）
...
class AutoRepair:
    ...
    def repair_start_gateway(self) -> RepairResult:
        ...
        if venv_python:
            ...
            project_dir = self._find_project_dir()
            subprocess.Popen(
                [str(venv_python), "-m", "museclaw.gateway.server"],
                cwd=str(project_dir),
                env={
                    **os.environ,
                    "PYTHONPATH": str(src_dir),
                    "MUSECLAW_HOME": str(self.home),
                },
    ...
    def repair_reinstall_daemon(self) -> RepairResult:
        ...
        # 需要 installer 模組
        try:
            from museclaw.installer.daemon import DaemonConfigurator
            from museclaw.installer.models import InstallConfig
    ...
    def repair_rebuild_dashboard(self) -> RepairResult:
        ...
        try:
            from museclaw.installer.electron import ElectronPackager
