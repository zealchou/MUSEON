...
將 MuseClaw 專案打包為單一 .command 自解壓安裝檔
"""
...
class InstallerPackager:
    ...
    def preserve_user_data(self, install_dir: Path) -> Dict[str, Path]:
        ...
        Returns:
            備份映射 {原始相對路徑: 臨時備份路徑}
        """
        backup_dir = Path(tempfile.mkdtemp(prefix="museclaw-backup-"))
...
#
# MuseClaw Self-Extracting Installer
# 雙擊此檔案即可在你的 Mac 上安裝 MuseClaw
#
# 此檔案包含完整的 MuseClaw 原始碼（Base64 編碼）
# 解壓後自動執行 BDD 安裝流程
#
...
echo ""
echo "  ╔═══════════════════════════════════════════╗"
echo "  ║  🐾  MuseClaw Self-Extracting Installer   ║"
echo "  ╚═══════════════════════════════════════════╝"
echo ""

PAYLOAD_LINE=__PAYLOAD_LINE__
DEFAULT_DIR="$HOME/MuseClaw"
...
# 如果環境變數已指定，直接使用
if [ -n "${MUSECLAW_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MUSECLAW_INSTALL_DIR"
    ...
    if command -v osascript &>/dev/null; then
        ...
        PARENT_DIR=$(osascript -e '
            ...
            try
                POSIX path of (choose folder with prompt "選擇 MuseClaw 安裝位置（會在此建立 MuseClaw 資料夾）：")
            on error
                ""
            end try
        ...
        if [ -n "$PARENT_DIR" ]; then
            ...
            if [ "$BASENAME" = "MuseClaw" ] || [ "$BASENAME" = "museclaw" ]; then
                INSTALL_DIR="${PARENT_DIR%/}"
            else
                INSTALL_DIR="${PARENT_DIR%/}/MuseClaw"
            fi
        fi
    fi
...
# ─── [5/5] 啟動 BDD 安裝流程 ───
echo "  [5/5] 啟動 MuseClaw 安裝流程..."
echo ""

export MUSECLAW_HOME="$INSTALL_DIR"
