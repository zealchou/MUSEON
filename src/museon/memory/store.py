"""Markdown-based memory storage for MUSEON.

Stores memories in human-readable Markdown format organized by date and channel.
This allows the boss to read MUSEON's memories directly.
...
class MemoryStore:
    ...
    async def load_daily_log(self, date: str) -> Dict[str, Any]:
        """Load all channel data for a specific date.

        Bridges the channel name convention:
        - File names use hyphens: meta-thinking.md
        - Dict keys use underscores: meta_thinking

        Args:
            date: Date string in format YYYY-MM-DD

        Returns:
            Dict with keys: meta_thinking, event, outcome, user_reaction
        """
        # Channel mapping: fusion key → file stem
        channel_map = {
        ...
        result = {}
        for fusion_key, file_channel in channel_map.items():
            entries = self.read(file_channel, date)
            result[fusion_key] = entries
    ...
    async def save_memory(
        ...
        """Save a memory entry (async wrapper for write).

        Args:
            channel: Channel name (e.g. 'meta_thinking' or 'meta-thinking')
        ...
        """
        # Normalize channel name: underscores → hyphens for file storage
        file_channel = channel.replace("_", "-")
