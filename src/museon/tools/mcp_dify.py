"""Dify MCP Bridge — MUSEON 原生 Dify 工作流整合.

透過 Dify REST API 讓 MUSEON 能自主駕馭 Dify 工作流：
- 列出可用工作流與應用
...
設計原則：
- MUSEON 是大腦（思考、診斷、策略），Dify 是手腳（執行、分發、排程）
- 此模組讓 MUSEON 能自主決定何時調用 Dify 工作流
...
DIFY_TOOLS = [
    {
        ...
        "description": (
            "執行 Dify 工作流。MUSEON 可透過此工具自主調用預建的 Dify 自動化工作流，"
