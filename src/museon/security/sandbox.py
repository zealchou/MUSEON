...
class Sandbox:
    ...
    def __init__(self, workspace_dir: Union[str, Path, None] = None):
        ...
        if workspace_dir is None:
            self.workspace_dir = Path.home() / ".museon" / "workspace"
    ...
    async def execute_command(self, command: str) -> Dict[str, Any]:
        ...
        # Command is whitelisted — 委派到 ExecutionSandbox（Docker）
        try:
            from museon.security.execution_sandbox import ExecutionSandbox
            sandbox = ExecutionSandbox(self.workspace_dir)
        ...
        # Fallback: 白名單通過但無 Docker（原有行為）
        logger.info(f"Command allowed (native fallback): {cmd_name}")
        return {
            "allowed": True,
            "executed": True,
            "command": cmd_name,
