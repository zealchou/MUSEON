Feature: 免疫系統 - 六層安全防禦
  MUSEON 的六層安全架構防禦各種攻擊，特別是 Prompt Injection

  Background:
    Given MUSEON 安全系統已啟動
    And 信任等級系統已初始化
  ...
  Scenario: 白名單命令可以執行
    ...
    When MUSEON 嘗試執行 "git status"
    Then 命令應該被允許
    And 命令應該成功執行

  Scenario: 非白名單命令被阻擋
    Given 命令不在白名單中
    When MUSEON 嘗試執行 "rm -rf /"
    Then 命令應該被禁止
  ...
  Scenario: 低信心決策被阻擋
    Given MUSEON 對某決策的信心度只有 55%
    When 信心度閾值是 70%
    Then 決策應該被阻擋
    And 應該詢問老闆意見

  Scenario: 多路徑推理驗證
    Given MUSEON 用三條推理路徑分析任務
    When 三條路徑的結論不一致
    Then 決策應該被延遲
    And 應該要求人工介入
