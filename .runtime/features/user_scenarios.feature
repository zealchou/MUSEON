Feature: 使用者情境壓力測試 — 200+ Real-World Scenarios
  ...
  Scenario: MOD-01 Gateway 模組載入成功
    When 驗證 "museon.gateway.server" 的 "create_app"
    Then 應回傳 SUCCESS
  ...
  Scenario: DMN-03 生成 plist 包含環境變數
    When 生成 plist
    Then 應包含 PYTHONPATH 和 MUSEON_HOME
  ...
  Scenario: DMN-18 plist 路徑正確性
    ...
    Then plist_path 應為 ~/Library/LaunchAgents/com.museon.gateway.plist
  ...
  Scenario: WIZ-02 首次啟動 — .env 缺少 SETUP_DONE
    Given .env 有 API key 但沒有 MUSEON_SETUP_DONE
    When 判斷是否為首次啟動
    Then 應回傳 True

  Scenario: WIZ-03 非首次啟動 — 已完成設定
    Given .env 有所有 key 和 MUSEON_SETUP_DONE=1
    When 判斷是否為首次啟動
    Then 應回傳 False

  Scenario: WIZ-04 SETUP_DONE=0 的語義問題
    Given .env 有 MUSEON_SETUP_DONE=0
    When 判斷是否為首次啟動
  ...
  Scenario: WIZ-28 標記設定完成
    When 標記設定完成
    Then .env 應包含 MUSEON_SETUP_DONE=1
  ...
  Scenario: MDL-09 InstallConfig 自動初始化路徑
    Given project_dir 為 /tmp/museon
    When 建立 InstallConfig
    Then venv_dir 應為 /tmp/museon/.venv
