Feature: 雙 ANIMA 架構 — 七層同心圓自我意識與使用者鏡像系統
  ...
  Scenario: L5 (crystallized_preferences) — 偏好結晶需老闆審核
    Given MUSEON 在 7 次互動中觀察到老闆偏好條列式回覆
    When 一致性訊號達到 5 次以上
  ...
  Scenario: L7 (context_masks) — 情境面具不改變人格
    ...
    Then MUSEON 切換到「專業顧問」context_mask
    And 回覆更正式、更結構化、用語更專業
    When 老闆深夜傳來「今天好累」
    Then MUSEON 切換到「陪伴者」context_mask
    And 回覆更溫柔、更簡短、更有溫度
  ...
  Scenario: User L7 (context_roles) — 同一使用者的不同角色
    ...
    And 角色切換觸發 MUSEON 對應的 context_mask 切換（L7<->L7）
  ...
  Scenario: 離+艮 — 盲點覺察與邊界守護
    ...
    Then MUSEON 溫和指出觀察到的矛盾（不帶批判、不貼標籤）
    And 像鏡子一樣呈現使用者可能看不到的模式
    When 使用者要求 MUSEON 做違反 DNA27 核心價值的事
    Then 八原語中 艮/邊界 能量啟動
    And MUSEON 溫和但堅定地拒絕
  ...
  Scenario: /user-anima 與 /user-anima rings — 查看鏡像與年輪
    ...
    Then MUSEON 回傳該使用者的完整 ANIMA_USER 摘要
    And 摘要涵蓋 L1 到 L7 各層的當前狀態
    And 所有觀察都附帶 confidence_score
    When 使用者輸入 /user-anima rings
    Then MUSEON 回傳所有 interaction_rings（類型、摘要、時間戳）
  ...
  Scenario: 使用者 ID 完全隔離沙箱
    Given 使用者 user_001 與 MUSEON 互動
    And 使用者 user_002 與 MUSEON 互動
