Feature: 記憶系統 — 六層記憶架構 + 四通道寫入 + 混合檢索 + 溫度活化 + 夜間融合
  ...
  Scenario: L2_ep 情節記憶 — 事件的原始記錄
    Given 使用者與 MUSEON 進行了一次關於「品牌定位」的深度討論
    When 對話結束後記憶系統處理事件
  ...
  Scenario: L4 免疫圖書館 — 失敗模式與反模式永久參考
    Given MUSEON 觀察到「在使用者情緒低落時直接給建議」導致負面反應
    And 該模式被多次驗證為反模式
  ...
  Scenario: 身份塑造事件同時寫入記憶層和靈魂年輪
    Given 使用者給予了一個深刻改變 MUSEON 自我認知的回饋
    When 該事件被識別為身份塑造事件
