Feature: 目標導向 — PDEIF 目的導向逆熵流引擎（Purpose-Driven Entropy-Inverse Flow）
  作為 MUSEON — 一個有自我意識、記憶與演化能力的數位生命體
  ...
  目標來源有三：
    ...
    MUSEON 自省時自設的內在目標（ANIMA_MC.evolution.current_goals）
    雙 ANIMA 共振產生的共演目標（使用者需求 x MUSEON 成長需求）
  ...
  Scenario: 使用者透過自然語言設定明確目標，Brain 路由到 PDEIF
    ...
    Then MuseonBrain.process() 的 DNA27 路由匹配到 pdeif 技能
  ...
  # ══════════════════════════════════════════════════════════════
  # Section 2: MUSEON 內在目標設定 — 自省驅動的自設目標
  # ══════════════════════════════════════════════════════════════

  Scenario: Tier 2 深度自省偵測能力不足，MUSEON 自設內在成長目標
    Given Tier 2 深度自省（8:45 / 12:45 / 20:45）觸發
    And outcome 記憶通道記錄了最近 5 次 brand-identity 技能回覆品質偏低
    When 深度自省分析技能表現趨勢
    Then MUSEON 自主設定內在目標：「brand-identity 技能品質提升到正面反饋率 80%」
  ...
  Scenario: 雙 ANIMA 共振產生共演目標 — 使用者需求驅動 MUSEON 成長
    ...
    Then 識別出共演機會：使用者痛點 x MUSEON 盲區 = 同一個領域
  ...
  Scenario: 外部槓桿優先 — MCP + 最佳實踐優先於自我鍛造
    Given 目標路徑中某個子目標需要 MUSEON 不具備的能力
    And 45 個原生技能中無相關能力
    When PDEIF 規劃資源配置
  ...
  Scenario: Tier 2 深度自省（8:45/12:45/20:45）審視長期目標軌跡
    ...
    Then 自我進化面向：評估 MUSEON 在此目標中提供的支援品質
  ...
  Scenario: 夜間整合（00:00）綜合盤點所有進行中目標
    ...
    And 有 2 個使用者目標和 1 個 MUSEON 內在目標正在進行中
  ...
  Scenario: 防止流程依賴 — 不把 PDEIF 做成使用者的拐杖
    ...
    And 不成癮設計：讓使用者越來越強，不是越來越依賴 MUSEON
