"""Naming Ceremony - First interaction with MUSEON.

Based on plan-v7.md Chapter 3:
- MUSEON's birth ritual
...
class NamingCeremony:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        Args:
            data_dir: Directory for storing ceremony state and ANIMA files
        """
        self.data_dir = Path(data_dir)
        self.data_dir.mkdir(parents=True, exist_ok=True)

        # Dual ANIMA: one for MUSEON, one for User
    ...
    def _parse_boss_answers(self, answers: str) -> Dict[str, Any]:
        ...
        Uses regex patterns to extract: boss name, business type,
        immediate need, main pain point from natural language answers.
        ...
        # ── 解析 Boss 稱呼 ──
        # 否定前綴（v9.0 修正：「不要叫我X」不應把 X 當名字）
        _NEGATION_PREFIXES = ["不要", "別", "不用", "不是", "不要再", "別再", "不必"]
        name_patterns = [
            r'叫我(.{1,10}?)(?:\s|$|，|,|。|！|\n)',
            r'我(?:的名字)?(?:是|叫)(.{1,10}?)(?:\s|$|，|,|。|！|\n)',
            r'稱呼我(.{1,10}?)(?:\s|$|，|,|。|！|\n)',
        ...
        for pat in name_patterns:
            m = re.search(pat, answers)
            if m:
                candidate = m.group(1).strip()
                # v9.0: 檢查是否有否定前綴（如「不要叫我分階段」）
                match_start = m.start()
                pre_text = answers[max(0, match_start - 5):match_start]
        ...
        for pat in need_patterns:
            m = re.search(pat, answers)
            if m:
                immediate_need = m.group(1).strip()
                break

        # ── 解析痛點 ──
        pain_patterns = [
            r'(?:最煩|最痛苦|最頭痛|困擾)(.{2,30}?)(?:$|。|\n)',
            r'(?:問題是|難題是|挑戰是)(.{2,30}?)(?:$|。|\n)',
        ]
        for pat in pain_patterns:
            m = re.search(pat, answers)
            if m:
                main_pain_point = m.group(1).strip()
                break
    ...
    def _generate_completion_message(
        ...
        Returns:
            Completion message
        """
        my_name = self._state.get("my_name", "MUSEON")
    ...
    def get_system_prompt_identity(self) -> str:
        ...
        if not anima_l1:
            return "You are MUSEON, an AI assistant waiting to be named."
