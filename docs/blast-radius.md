...
> **v2.09 (2026-04-06)**：x-ray 透視引擎新增——新增 🟢 綠區：`skills/native/x-ray/`（x-ray 透視引擎 Skill，扇入=1：user 觸發，扇出=4：knowledge-lattice diagnostic_crystal 寫入 + dse 方案驗證 + fix-verify 驗收閉環 + plan-engine 修復行動計畫化，安全分級 🟢 綠區，無共享狀態直接寫入，透過 knowledge-lattice 落地）。同步 system-topology v1.91、memory-router v1.31。
> **v2.05 (2026-04-05)**：五虎將自癒管線四修復藍圖同步——`doctor/musedoc.py` rolled_back 分支新增 update_status("rolled_back")（與 fixed_by_musedoc/needs_human 分支對稱）；`doctor/runbook.py` 新增 RB-010（🟢 綠區，扇入=0，`RB010_SQLiteCorruption`，C→B→A 瀑布式降級修復，SAFE_DBS 白名單限制，timeout=120s），ALL_RUNBOOKS 由 8→9 條；`doctor/museoff.py` L3 probe blast_origin.file 從 detail（錯誤訊息）改為 name（路徑），L6c probe 兩個 _create_finding 補齊 `blast_origin=BlastOrigin(...)` + `prescription=Prescription(...)` 物件（修復原本傳字串的 bug）；`doctor/musedoc.py` `_auto_expire_stale()` 新增（CRITICAL + 7 天 + pgrep 確認服務已恢復 → update_status("fixed_externally")），由 nightly_surgery() 在 load_open() 後呼叫；RB-007 post_check 改為先 pgrep 早返回 True，再 asyncio.sleep(45)，最後無條件 True。扇入扇出不變（純內部修復）。同步 system-topology v1.88。
...
> **v2.08 (2026-04-06)**：brain.py ctx.user_id 修復 + C-traits 提取 + telegram.py Skill 缺口/重鍛 Inline Keyboard——`agent/brain.py` 三處 `ctx.user_id` 從 v1 殘留改為直接使用 `user_id` 參數（修復荒謬雷達 + 多星座雷達靜默失敗）；新增 `_c_traits` 提取邏輯（從 ANIMA_MC.personality.trait_dimensions 提取 C1-C5，作為 skill_router.match() 的 `c_traits` 參數輸入，扇出不變）；`channels/telegram.py` `_on_skill_gap_proposal` 重構為非同步帶 Inline Keyboard 的發送流程（新增 `_send_skill_gap_proposal_with_keyboard()` + 4 個 callback actions：`skill:gap_approve`/`skill:gap_ignore`/`skill:reforge_approve`/`skill:reforge_ignore`，`_handle_skill_callback` 路由分支 4→8）；gap_approve/reforge_approve 寫入 `_system/skill_requests/` 並更新 status 為 `pending_dse_confirmed`，gap_ignore/reforge_ignore 刪除對應 req 檔；brain.py 扇入扇出不變（純 bug 修復 + C-traits 提取為現有 skill_router.match 參數擴充）；telegram.py 扇入不變，callback 路由分支 +4；`_system/skill_requests/` 共享狀態新增讀寫方向（telegram.py gap_approve/reforge_approve 寫入 status 欄位，gap_ignore/reforge_ignore 刪除 req 檔，之前只有 gap_accumulator 寫入新檔）。同步 system-topology v1.90、joint-map v1.75。
...
`cache/semantic_response_cache.py`（★ v2.00 新增，扇入=2（brain.py query、l4_cpu_observer write），扇出=1（Qdrant semantic_response_cache collection），Qdrant 語意快取，零 LLM。v12 新增），
...
`gateway/cron_registry.py`（★ v1.64 從 server.py 拆出，扇入=1（server.py），1424 行；系統 cron 任務註冊，含五虎將 + Nightly + 41 項排程；內含 quota circuit breaker（v12，quota 耗盡時跳過 LLM cron jobs）），
...
| 2026-04-06 | v2.09 | x-ray 透視引擎新增——新增 🟢 綠區：x-ray Skill（扇入=1：user 觸發，扇出=4：knowledge-lattice/dse/fix-verify/plan-engine）；安全分級 🟢 綠區；無新共享狀態（診斷結晶透過 knowledge-lattice 落地）。同步 system-topology v1.91、memory-router v1.31 |
| 2026-04-06 | v2.08 | brain.py ctx.user_id 修復 + C-traits 提取 + telegram.py Skill 缺口/重鍛 Inline Keyboard——brain.py 三處 ctx.user_id→user_id（v1 殘留，修復荒謬雷達+多星座雷達靜默失敗）；brain.py 新增 _c_traits 提取（ANIMA_MC.trait_dimensions C1-C5 → skill_router.match c_traits 參數）；telegram.py _on_skill_gap_proposal 重構為 Inline Keyboard 非同步流程，新增 _send_skill_gap_proposal_with_keyboard()，4 個 callback actions（skill:gap_approve/gap_ignore/reforge_approve/reforge_ignore）；gap_approve/reforge_approve 寫入 _system/skill_requests/ status=pending_dse_confirmed，gap_ignore/reforge_ignore 刪除 req 檔；brain.py 扇入扇出不變；telegram.py 扇入不變，_handle_skill_callback 分支 4→8；_system/skill_requests/ 共享狀態新增 telegram.py 讀寫路徑（之前只有 gap_accumulator 寫入）。同步 system-topology v1.90、joint-map v1.75 |
