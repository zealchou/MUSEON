...
#
# 使用方式:
#   ./install-museon.sh [path/to/MUSEON.app]
#
...
if [ -z "$1" ]; then
    # 在當前目錄尋找 MUSEON.app
    if [ -d "MUSEON.app" ]; then
        APP_PATH="$(pwd)/MUSEON.app"
    elif [ -d "electron/dist/mac/MUSEON.app" ]; then
        APP_PATH="$(pwd)/electron/dist/mac/MUSEON.app"
    elif [ -d "electron/dist/mac-arm64/MUSEON.app" ]; then
        APP_PATH="$(pwd)/electron/dist/mac-arm64/MUSEON.app"
    else
        error "找不到 MUSEON.app。請提供路徑作為參數。"
    fi
else
    APP_PATH="$1"
fi
...
# 目標路徑
TARGET_PATH="/Applications/MUSEON.app"

# 如果已經安裝,詢問是否覆蓋
if [ -d "$TARGET_PATH" ]; then
    warn "MUSEON 已安裝在 /Applications"
...
# 複製到 /Applications
info "複製 MUSEON.app 到 /Applications..."
...
# 檢查 .app 結構
if [ ! -f "$TARGET_PATH/Contents/MacOS/MUSEON" ] && \
   [ ! -f "$TARGET_PATH/Contents/MacOS/MUSEON Dashboard" ]; then
    error "應用程式結構異常"
fi
...
# 詢問是否立即啟動
echo ""
read -p "是否立即啟動 MUSEON? (Y/n): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Nn]$ ]]; then
    info "啟動 MUSEON..."
    open "$TARGET_PATH"
...
echo "   MUSEON 安裝完成！"
echo ""
echo "   應用位置: /Applications/MUSEON.app"
echo ""
